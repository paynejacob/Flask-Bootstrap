...
class AngularTemplateCacheFilter(Filter):
  ...
  template_start = ("angular.module('%s', [])"
                    ".run(['$templateCache', function($templateCache) {\n")
  template_file = "  $templateCache.put('{rel_path}', {escaped_html});\n"
  template_end = "}]);\n"

  def input(self, _in, out, source_path=None, **_):
    ...
    out.write(self.template_file.format(rel_path=rel_path,
                                        escaped_html=json.dumps(html_template)))

  def output(self, _in, out, output_path=None, **_):
